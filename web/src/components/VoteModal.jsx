...
export default function VoteModal({
  ...
  const normalized = useMemo(() => {
    ...
    return arr.map((opt, i) =>
      typeof opt === 'string'
        ? { num: i + 1, label: opt }
        : { num: opt?.num ?? i + 1, label: opt?.label ?? String(opt?.num ?? i + 1) }
    );
  }, [options]);
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => (firstButtonRef.current?.focus() || inputRef.current?.focus()), 0);
    } else {
      setChoice('');
    }
  }, [open]);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 id="vote-modal-title" className="text-xl font-semibold">🗳️ Topic Vote</h2>
          <button
            className="text-sm px-2 py-1 rounded border hover:bg-white"
            onClick={onClose}
            disabled={busy}
          >
            Close
        ...
        <ol className="list-decimal pl-6 space-y-1 mb-4">
          {normalized.map((opt) => (
            <li key={opt.num}><span className="font-medium">{opt.label}</span></li>
          ))}
        </ol>
